function [P,P0, output] = cp_wopt_s27(Z,W,R,A_S,varargin)
% Copy of cp_wopt_s26 with changes as:
% CP_WOPT for tensor factorisation, when the spectra is fixed and
% concentration need to be calibrated.
...
if use_lbfgsb
    lower = convert_bound(lower,sz(1:end-1),R); %=======================Added (1:end-1)================>
    upper = convert_bound(upper,sz(1:end-1),R); %=======================Added (1:end-1)================>
end
...
if (isequal(funtype,'auto') && isa(Z,'tensor')) || isequal(funtype,'default')
    funhandle = @(x) tt_cp_wfun(Z,W,x,normZsqr,A_S);%============================Added Ad ====================>
else
    ...
    funhandle = @(x) tt_cp_wfun(Zvals,W,x,normZsqr,A_S,fflag);%==========================Added Ad =================>
end
...
function [f,g] = tt_cp_wfun(Zdata,W,x,normZsqr,A_S,memflag) %=====================================Added Ad==============>
