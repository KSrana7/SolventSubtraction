...
filename_ftir = 'Data/PEB_ftir.xlsx';
...
%----Solvent----%
filename_ftir_water = 'Data/Water_Samples/FTIR_Water_nist.xlsx';
...
% jj
%baseline and background correction 200,200
...
figure();plot(lam_ftir,Data_ftir); grid on;legend(string([1,2,3,4,5,6,7,8,9]))
...
Dback_ftir_s(split+1:end,:)=msbackadj(lam_ftir_water(split+1:end),Dback_ftir_s(split+1:end,:),'WindowSize',800); % Correct baseline of signal--> spline approximation--> noise removal
...
% figure();plot(lam_ftir_water2,D_ftir_water2,lam_ftir_water,Dback_ftir_s,lam_ftir,7*Data_ftir(:,1));grid on
