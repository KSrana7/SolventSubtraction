function [P,P0,mu_l_o, output] = cp_wopt_s26(Z,W,R,Ad,varargin)
% Copy of cp_wopt_s24 with changes as:
% CP_WOPT for solvent extraction when solvent signature profile is known,
% but scaling factor is unknown AND when the solvent is added as extra
% latent factor in the deconvolution. WITH
% orthogonal condition on the solvent vs other latent factor AND
% with langragain way of adaptive constraint handling
...
elseif isa(init,'ktensor')
    ...
    if strcmpi(init,'nvecs')
        ...
        for n=1:N
            P0{n} = matrandnorm(feval(init,sz(n),R+R2)); %===========Added +R2 for solvent=========>
        end
        
    end
end

% N_s = numel(P0{1})+numel(P0{2});
%==================================================================================%
...
if use_lbfgsb
    lower = convert_bound(lower,sz,R+R2);%================================Added +R2===========================>
    upper = convert_bound(upper,sz,R+R2);%================================Added +R2==========================>
end
%===========================Added====================>
lower=lower(1:end-size(Ad,1)*R2);
upper=upper(1:end-size(Ad,1)*R2);

lower=vertcat(lower,1.0e0);
upper=vertcat(upper,Inf);
...
if (isequal(funtype,'auto') && isa(Z,'tensor')) || isequal(funtype,'default')
    funhandle = @(x) tt_cp_wfun(Z,W,x,normZsqr,Ad,R);%============================Added Ad ====================>
else
    ...
    funhandle = @(x) tt_cp_wfun(Zvals,W,x,normZsqr,Ad,fflag);%==========================Added Ad =================>
end
...
%     P = ktensor(tt_cp_vec_to_fac(xx, Z));
    ...
    P = cell(N,1);
    for n = 1:N-1
        idx1 = sum(sz(1:n-1))*(R+R2) + 1;
        idx2 = sum(sz(1:n))*(R+R2);
        P{n} = reshape(xx(idx1:idx2),sz(n),(R+R2));
    end
    idx1 = sum(sz(1:N-1))*(R+R2) + 1;
    idx2 = sum(sz(1:N))*(R+R2)-size(Ad,1)*R2;
    P{N} = reshape(vertcat(xx(idx1:idx2),Ad),sz(N),R+R2);
    P = ktensor(P);
    mu_l_o = xx(idx2+1);

%=========================================>
%     P = ktensor(P); %===================replaced ktensor(tt_cp_vec_to_fac(xx, Z))==========================>
...
% P = fixsigns(P);
%====================================================>
function [f,G] = tt_cp_wfg(Z,W,A,normZsqr,Ad,mu_l) %=========================================Added Ad & mu_l================>
...
% function value
% A_S = reshape(kron(S,Ad), [numel(Ad), size(S)]); %==========================Added============================>
% A_S = permute(A_S,[2,3,1]);
% A_S_W = W.*full(ktensor(S));%W.*A_S;     %=======================================================Added==============================>

f_1 = (0.5 * normZsqr - innerprod(Z,B) + 0.5 * norm(B)^2);

dot_p = Ad'*A{N}(:,1:end-1);
scale_p = (norm(Ad).*diag(A{N}(:,1:end-1)'*A{N}(:,1:end-1))).^0.5;
dot_p_s = dot_p./scale_p';
dot_p_ss = sum(dot_p_s);%sum(Ad'*A{N}(:,1:end-1)/);

f_2 =  mu_l*dot_p_ss;%mu_l*norm(Ad'*A{N}(:,1:end-1))^2/(norm(Ad)*norm(A{N}(:,1:end-1)))^2;%mu_l*norm(Ad'*A{N})^2/(norm(Ad)*norm(A{N}))^2;%0.5*(norm(Ad'*A{N}))^2;%- innerprod((Z-B),A_S_W) + 0.5*norm(A_S_W)^2; %=========================================================>

f = f_1 + f_2;
% f = 0.5 * normZsqr - innerprod(Z,A_S_W) + 0.5 * norm(A_S_W)^2;  %=========================================================>
...
for n = 1:N
    ...
    if n==N %============================Added=========================
        ...
        G{n}(:,1:size(A{N},2)-1) = G{n}(:,1:size(A{N},2)-1) + mu_l*Ad/norm(Ad)^0.5;%./scale_p';%;%sum(scale_p);%;%sum(scale_p);;
    end
end

% ============================Added===================================================>
% 
% G{1} = -mttkrp(T,S,1);%-norm(T)*ones(size(S{1})); %===========changed to 1 from N+1
% G{2} = -mttkrp(T,S,2);%-norm(T)*ones(size(S{2})); %===========changed to 2 from N+2
%====================================================================================>

function [f,g] = tt_cp_wfun(Zdata,W,x,normZsqr,Ad,R,memflag) %=====================================Added Ad==============>
...
for n = 1:N-1
    idx1 = sum(sz(1:n-1))*(R+R2) + 1;
    idx2 = sum(sz(1:n))*(R+R2);
    A{n} = reshape(x(idx1:idx2),sz(n),(R+R2));
end
idx1 = sum(sz(1:N-1))*(R+R2) + 1;
idx2 = sum(sz(1:N))*(R+R2)-size(Ad,1)*R2;
A{N} = reshape(vertcat(x(idx1:idx2),Ad),sz(N),R+R2);
...
if isa(Zdata,'tensor') || isa(Zdata,'sptensor')
    ...
    [f,G] = tt_cp_wfg(Zdata,W,A,normZsqr,Ad,mu_l);%===========================================Added Ad & mu_l====================>
else
    if ~exist('normZsqr','var')
...
g = tt_fac_to_vec(G); 
g = g(1:end-size(Ad,1)*R2);%=====================Added========================>
g = vertcat(g,sum(Ad'*A{N}));
