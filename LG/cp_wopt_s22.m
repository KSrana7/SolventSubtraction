function [P,P0,S_O, output] = cp_wopt_s22(Z,W,R,Ad,varargin)
% CP_WOPT for solvent extraction when solvent signature profile is known,
% but scaling factor is unknown and when the latent data deconvolution is
% ignored
...
% end
%===================================================================>
%======================================= Addded Intialisation for S=============== % 
%     Ad_m = rand();zeros(size(Z));
%     Ad_m(:,1) = Ad
if iscell(init)
    P0 = init;
elseif isa(init,'ktensor')
    ...
    R2 = size(Ad,2);
    P0{1} = rand(sz(1),R); %=================chnged to 1 from N+1=====>
    P0{2} = rand(sz(2),R); %=================chnged to 2 from N+2=====>
end
...
if use_lbfgsb
%     lower = convert_bound(lower,sz,R);%================================Commented===========================>
%     upper = convert_bound(upper,sz,R);%================================Commented==========================>
    lower = 0.0*ones(sum(sz(1:2)*R),1);  %======================================Added========================
    upper = Inf*ones(sum(sz(1:2)*R),1);  %======================================Added========================
end
...
if use_lbfgsb
    ...
    %============================================================Added================>
%     P_O = tt_cp_vec_to_fac(xx(1:end-N_s),Z);  %========================Added (1:end-N_s)========================================>
%     S_O{1} = reshape(xx(end-N_s+1:end),sz(1),sz(2)); %=======================Added===============================>
    P={};
    S_O{1} = reshape(xx(end-N_s+1:end-sz(2)*R),sz(1),R); %=======================Added===============================>
    S_O{2} = reshape(xx(end-sz(2)*R+1:end),sz(2),R); %=======================Added===============================>
    S_O{3} = repmat(Ad,1,R);
    S_O = ktensor(S_O);
    %=================================================================================>
%     P = ktensor(P_O); %===================replaced ktensor(tt_cp_vec_to_fac(xx, Z))==========================>
...
% P = fixsigns(P);
%====================================================>
function [f,G] = tt_cp_wfg(Z,W,A,normZsqr,Ad,S) %=========================================Added Ad & S================>
...
% f = 0.5 * normZsqr - innerprod(Z,B) + 0.5 * norm(B)^2 - innerprod((Z-B),A_S_W) + 0.5*norm(A_S_W)^2; %=========================================================>
...
sz = size(Zdata);   %============================Added==================>
N_s = sz(1)*R+sz(2)*R;
A = Ad;%tt_cp_vec_to_fac(x(1:end-N_s),W);  %========================Added (1:end-N_s)========================================>
S{1} = reshape(x(end-N_s+1:end-sz(2)*R),sz(1),R); %=======================Added===============================>
S{2} = reshape(x(end-sz(2)*R+1:end),sz(2),R); %=======================Added===============================>
S{3} = repmat(Ad,1,R);
...
if isa(Zdata,'tensor') || isa(Zdata,'sptensor')
    ...
    [f,G] = tt_cp_wfg(Zdata,W,A,normZsqr,Ad,S);%===========================================Added Ad,S====================>
else
    if ~exist('normZsqr','var')
...
g = tt_fac_to_vec(G); %===========================changed fuction================================>
